...
%% kernel recovery
use_GLMI = logical(f2_option(f2_options, "use_GLMI", true));
if use_GLMI
    default_label = "GLMI";
else
    default_label = "GLMS";
end
label = string(f2_option(f2_options, "output_label", default_label));
...
compute_table_metrics = logical(f2_option( ...
    f2_options, "compute_table_metrics", false));
validateattributes(compute_table_metrics, {'logical'}, {'scalar'});
clear f2_options default_lr_schedule default_batch_size default_label requested_nte;
