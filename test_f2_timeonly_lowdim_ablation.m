...
% thedoc = "test_f2_timeonly_lowhdim_ablation";
...
min_b = 0.01; %0.03;
min_b_List = [0,0.005,0.02];
B_weight = 0.1; % 0.1
B_weight_List = [0,0.05,0.2];
smooth_weight = 0.2; % 1*h^2
smooth_weight_List = [0, 0.1, 0.4];
mode = "eliminate";
num_para = 3;

ablation_type = ["lowerbound","barrierweight","smoothweight"];
...
for itype = 1:3
    ...
    for ipara = 1:num_para
        if itype == 1
            min_b = min_b_List(ipara); 
            B_weight = 0.1; 
            smooth_weight = 0.2; 
        elseif itype == 2
            min_b = 0.01; 
            B_weight = B_weight_List(ipara); 
            smooth_weight = 0.2; 
        elseif itype == 3
            min_b = 0.01; 
            B_weight = 0.1; 
            smooth_weight = smooth_weight_List(ipara); 
        end
        ...
        %% kernel
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),".mat"), "X");
        ...
        exportgraphics(ax,strcat(theplot,thedoc,"_EstKer",label,ablation_type(itype),"ipara_",string(ipara),".pdf"));  
        ...
        kernel_rela_err'
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_EstKerRelErr.mat"), "kernel_rela_err");
        
        %% mu
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_mu.mat"), "mu");
        ...
        exportgraphics(ax,strcat(theplot,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_mu_all",".pdf")); 
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_mu_all.mat"), "mu_all");
        ...
        fprintf(['-----Mu Error Table----- \n' ...
            'Line 1: Rel l1, Rel l2, Rel l inf\n'])
        mu_rela_err'
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_EstMuRelErr.mat"), "mu_rela_err");
        ...
        figH = gcf;
        set(figH, 'Units', 'points','OuterPosition', [0 0 235 235]) % standard size: 19.7 17.5
        exportgraphics(ax,strcat(theplot,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_TrainLogLike",".pdf")); 
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_TrainLogLike",".mat"), "nll_all");
        ...
        mean(proberror,1)
        save(strcat(theout,thedoc,label,ablation_type(itype),"ipara_",string(ipara),"_ProbPredErr",".mat"), "proberror");
    end
end
