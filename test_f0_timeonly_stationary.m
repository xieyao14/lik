...
min_b = 0.01; %0.03;
B_weight = 0.1; % 0.1
smooth_weight = 0.004; % 1*h^2
mode = "eliminate";


if use_VI
    label = "VI";
    lr_schedule = [0.4*ones(20,1), 0.2*ones(20,1), 0.2*ones(20,1) ];
else
    label = "GD";
    lr_schedule = [0.2*ones(20,1), 0.1*ones(20,1), 0.1*ones(20,1) ];
end

bs_schedule = [400*ones(60,1)];
...
figure(2),clf;
box on;
plot(1:Nprime, psi(Nprime+1,1:Nprime),'-', 'linewidth', 2, 'Color', [0 0 0 0.3]);
grid on;
hold on;

plot(1:Nprime, X_GD(Nprime+1,1:Nprime),'-.', 'linewidth', 1, 'Color', [1,0,0,0.5]);
plot(1:Nprime, X_VI(Nprime+1,1:Nprime),'--', 'linewidth', 1, 'Color', [0,0,1,0.5]);
...
ax = gca;
xlabel('{Time delay} $j-i$','fontsize',8,'interpreter','latex');
ylabel('$\psi_{j-i}$','fontsize',8,'interpreter','latex');
drawnow();
figH = gcf;
set(figH, 'Units', 'points','OuterPosition', [0 0 235 235]) % standard size: 19.7 17.5
exportgraphics(ax,strcat(theplot,thedoc,"_StationaryKerCompare.pdf"));  
