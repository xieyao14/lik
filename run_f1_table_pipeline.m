function pipeline_report = run_f1_table_pipeline(options)
%RUN_F1_TABLE_PIPELINE Run and aggregate the current f1 method scripts.
%
...
fprintf("f1 pipeline: %d replicas, %d methods, %d epochs per method.\n", ...
...
[table_show, table_full, aggregation_report] = ...
    test_f1_table(aggregation_options);
...
fprintf("f1 pipeline finished: %d completed, %d skipped, %d failed.\n", ...
...
switch method
    ...
    otherwise
        error("TULIK:UnknownF1Method", "Unknown f1 method: %s", method);
end
end

function captured_output = run_current_tulik( ...
    method, replica_id, seed, config, script_dir)
source = fileread(fullfile(script_dir, "test_f1_timeonly_highdim.m"));
source = configure_output_directories(source, config);
source = replace_once(source, "clearvars -except f1_options;", ...
    "% Workspace clearing disabled by run_f1_table_pipeline.", ...
    "TULIK workspace setup");
source = truncate_before_return(source, 1, "TULIK"); %#ok<NASGU>

if method == "TULIK_VI"
    use_vi = true;
    full_schedule = [0.4*ones(100, 1); 0.2*ones(200, 1)];
else
    use_vi = false;
    full_schedule = [0.2*ones(100, 1); 0.1*ones(200, 1)];
end
...
f1_options = struct( ...
    ...
    'output_suffix', sprintf('_replica_%02d_TULIK_', replica_id), ...
...
function captured_output = run_current_glm( ...
    method, replica_id, seed, config, script_dir)
source = fileread(fullfile(script_dir, "test_f1_timeonly_highdim_GLM.m"));
source = configure_output_directories(source, config);
source = configure_legacy_common(source, seed, replica_id, config, "GLM");

use_glm_l = method == "GLM_L";
source = replace_once(source, "use_GLMI = 0;", ...
...
function captured_output = run_current_hpe( ...
    replica_id, seed, config, script_dir)
source = fileread(fullfile(script_dir, "test_f1_timeonly_highdim_HPE.m"));
source = configure_output_directories(source, config);
source = configure_legacy_common(source, seed, replica_id, config, "HPE");
...
source = replace_once(source, ...
    ...
    "% Unused eta_ob allocation omitted by run_f1_table_pipeline.", ...
    family + " unused eta allocation");
source = replace_once(source, ...
    "eta_ob(:, t:t+Nprime-1, t)= ypre;", ...
    "% Unused eta_ob assignment omitted by run_f1_table_pipeline.", ...
    family + " unused eta assignment");

switch family
    case "GLM"
        old_document = 'thedoc = "test_f1_timeonly_highdim_GLM";';
        new_document = sprintf( ...
            'thedoc = "test_f1_timeonly_highdim_replica_%02d_";', replica_id);
    case "HPE"
        old_document = 'thedoc = "test_f1_timeonly_highdim_HPE";';
        new_document = sprintf( ...
            'thedoc = "test_f1_timeonly_highdim_replica_%02d_HPE";', replica_id);
    otherwise
        error("TULIK:UnknownLegacyFamily", ...
            "Unknown legacy method family: %s", family);
end
...
source = [source(1:starts(1)-1), ...
    sprintf('%% Continued by run_f1_table_pipeline.\n'), ...
...
prefix = string(fullfile(output_dir, sprintf( ...
    'test_f1_timeonly_highdim_replica_%02d_', replica_id)));
