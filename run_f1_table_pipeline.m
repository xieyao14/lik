...
allowed_options = ["replica_ids", "methods", "base_seed", ...
    ...
    "batch_size", "output_dir", "plot_dir", "show_figures", "overwrite", ...
    "apply_low_rank_truncation", ...
...
config.show_figures = logical(pipeline_option(options, "show_figures", false));
config.overwrite = logical(pipeline_option(options, "overwrite", false));
config.apply_low_rank_truncation = logical(pipeline_option( ...
    options, "apply_low_rank_truncation", true));
...
logical_fields = ["show_figures", "overwrite", "continue_on_error", "print_latex", ...
    "save_summary", "verbose", "apply_low_rank_truncation"];
...
f1_options = struct( ...
    ...
    'use_VI', use_vi, ...
    'apply_low_rank_truncation', config.apply_low_rank_truncation); %#ok<NASGU>

addpath(script_dir);
captured_output = evalc('test_f1_timeonly_highdim;');
end
...
addpath(script_dir);
captured_output = evalc('test_f1_timeonly_highdim_GLM;');
end
...
f1_options = struct( ...
    ...
    'batch_size', config.batch_size, ...
    'output_document', sprintf( ...
        'test_f1_timeonly_highdim_replica_%02d_HPE', replica_id), ...
    'output_dir', config.output_dir, ...
    'plot_dir', config.plot_dir, ...
    'show_figures', config.show_figures); %#ok<NASGU>

addpath(script_dir);
captured_output = evalc('test_f1_timeonly_highdim_HPE;');
end
